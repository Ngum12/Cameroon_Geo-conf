...
def detect_language(text: str) -> str:
    ...
    Enhanced with common French/English patterns for better detection.
    In production, you might want to use langdetect or similar libraries.
    """
    text_lower = text.lower()
    
    # Enhanced French indicators - including common Cameroon French phrases
    french_words = [
        # Articles, pronouns
        "le", "la", "les", "de", "du", "des", "et", "à", "dans", "pour", "avec", "sur", "est", "sont", "une", "un",
        # Common words
        "merci", "beaucoup", "bonjour", "bonsoir", "salut", "comment", "allez", "vous", "très", "bien",
        "oui", "non", "peut", "être", "avoir", "faire", "tout", "tous", "aussi", "plus", "même", "après",
        # Cameroon-specific
        "cameroun", "yaoundé", "douala", "bamenda", "garoua", "maroua", "bafoussam", "ngaoundéré",
        "gouvernement", "politique", "sécurité", "développement", "économie"
    ]
    french_count = sum(1 for word in french_words if word in text_lower)
    
    # Enhanced English indicators
    english_words = [
        "the", "and", "to", "of", "in", "for", "with", "on", "is", "are", "a", "an", "this", "that",
        "hello", "thank", "you", "very", "much", "how", "what", "where", "when", "why", "good", "bad",
        "have", "has", "will", "would", "could", "should", "can", "may", "might", "must", "shall",
        # Cameroon English
        "cameroon", "government", "security", "development", "economy", "peace", "conflict"
    ]
    english_count = sum(1 for word in english_words if word in text_lower)
    
    # Enhanced detection logic with confidence threshold
    if french_count > english_count and french_count > 0:
        return "fr"
    elif english_count > french_count and english_count > 0:
        return "en"
    elif french_count == english_count and french_count > 0:
        # If equal counts, check for specific French patterns
        french_patterns = ["ç", "é", "è", "à", "ù", "ê", "ô", "î", "ï"]
        if any(pattern in text_lower for pattern in french_patterns):
            return "fr"
        else:
            return "en"
    else:
        return "fr"  # Default to French for Cameroon context
...
async def translate_text(text: str, source_lang: str = "auto") -> Dict[str, Any]:
    ...
    try:
        ...
        # Prepare languages for M2M100 format
        if source_lang == "fr":
            src_lang = "fr"
        else:
        ...
        # Perform translation with M2M100 pipeline
        logger.info(f"Translating from {src_lang} to {tgt_lang}: {text[:50]}...")
        
        # For M2M100, we need to use the tokenizer directly with proper language tokens
        ...
        # The result is now the decoded string
